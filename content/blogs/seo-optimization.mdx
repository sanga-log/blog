...
![구글 검색 결과 — 기존 smartscore.kr 도메인은 "이 페이지에 관한 정보가 없습니다"로 노출](/images/seo-search-google-before.png)

![네이버 검색 결과 — smartscore.global 도메인 정보가 노출되는 상태](/images/seo-search-naver-before.png)
...
![구글 사이트 소유 확인 — DNS에 google-site-verification TXT 레코드 추가](/images/seo-verify-google-dns.png)

### 네이버 서치 어드바이저

![네이버 사이트 소유 확인 — HTML 인증 파일을 프로젝트 루트에 업로드](/images/seo-verify-naver-html.png)
...
![Sitelinks 예시 — 메인 검색 결과 아래 하위 페이지 링크가 묶여 노출되는 구조화 데이터 형태](/images/seo-rich-sitelinks.png)

![구조화된 캐러셀 데이터 — 여러 항목이 가로로 나열되는 리치 결과 형태](/images/seo-rich-carousel.png)

BreadcrumbList 스키마를 `index.html`에 추가해 현재 페이지의 계층 구조를 구글에 전달했습니다.

![구글 공식 문서 주의사항 — 올바르게 마크업해도 구조화 데이터가 검색 결과에 표시되는 것은 보장되지 않음](/images/seo-structured-data-note.png)
...
![개선 전 상용 SEO 점수 — 83점 (메타 설명 누락, robots.txt 오류)](/images/seo-score-before.png)

![개선 후 상용 SEO 점수 — 92점](/images/seo-score-prod.png)

![로컬 SEO 점수 — 100점 (구조화 데이터 유효)](/images/seo-score-local.png)
