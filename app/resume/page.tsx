    ...
    <div className="mt-8 pt-8 border-t border-gray-100">
      ...
      <ul className="flex flex-col gap-1.5">
        {items.map((item, i) => (
          <li
            key={i}
            className="text-sm leading-relaxed text-gray-600 flex items-start gap-2"
          >
            <Dot />
            <span>{item}</span>
          </li>
    ...
    <div className="max-w-3xl mx-auto px-6 py-16">
      ...
      <section className="mb-14">
        ...
        <p className="text-base leading-relaxed text-gray-700">
          3년차 프론트엔드 개발자로서, 300명 규모의 조직에서 동료들과 합을
          맞춰가며 제품을 만든 경험과, 30명 규모의 조직에서 매장 사장님의
          목소리를 듣고 바로 고쳐가던 경험을 거치며, 서로 다른 환경에서의 협업을
          직접 느끼고 배웠습니다.
        </p>
      </section>
          ...
          <div>
            ...
            <div className="pl-6 ml-[52px]">
              <Project
                ...
                items={[
                  <>
                    &ldquo;무인 매장에서 사용하는 서비스인데, 정작 빌드·배포
                    프로세스에 수작업 의존도가 높다&rdquo;고 판단하여 단계적으로
                    자동화를 확장: 빌드 스크립트 분리 →{" "}
                    <code>--publish always</code>로 S3 자동 업로드 →
                    electron-updater Silent 설치로 매장 방문 제거 → MAC Address
                    기반 서버 API 조회로 매장·룸·경로 설정까지 자동화.{" "}
                    <B>
                      매장 설치·업데이트 시 현장 방문 및 수동 설정 완전 제거
                    </B>
                  </>,
                  <>
                    STOMP 장시간 연결(2시간+) 시 프레임 에러 발생 후 메시지
                    송수신이 완전히 중단되고, 재연결해도 세션 ID 불일치로 이전
                    대화 이력이 손실되는 문제 발견. 서버 로그와 클라이언트 세션
                    ID를 대조해 원인을 오래된 세션 재사용으로 특정하고, 재연결
                    시 세션을 새로 발급받도록 수정 및{" "}
                    <code>reconnectDelay</code> 최적화.{" "}
                    <B>평균 복구 시간 3초 이내, 대화 이력 손실 제거</B>
                  </>,
                  <>
                    스윙 영상 3개 업로드 중 AI 피드백이 비동기로 도착해 순서가
                    꼬이는 문제를 <B>메시지 버퍼링 큐 아키텍처</B>로 해결.
                    새로고침 시 기존 메시지가 재수신되며 순서가 뒤섞이는 문제도{" "}
                    <code>pageLoadTime</code> 기반 초기 로드 감지 +{" "}
                    <code>sessionStorage</code> 재생 이력 추적으로 동시 해결
                  </>,
                  <>
                    무인 매장 특성상 터미널 접근이 불가능한 환경에서{" "}
                    <code>console.log</code>·<code>electron-log</code> 혼용으로
                    주요 모듈 로그가 파일에 기록되지 않는 사각지대를 발견.
                    카테고리별 로거 팩토리(<code>shopInfoLog</code>,{" "}
                    <code>fileWatcherLog</code> 등)로 전 모듈 로깅을 통일하고
                    렌더러(React) 에러를 IPC 채널로 메인 프로세스에 수집. 일별
                    로그 로테이션 + 7일 자동 삭제로{" "}
                    <B>
                      현장 방문 없이 앱 전체 상태를 단일 로그 파일에서 파악
                      가능한 구조 확립
                    </B>
                  </>,
                  <>
                    619줄짜리 모놀리식 <code>electron.js</code>를{" "}
                    <code>ipc/</code>·<code>utils/</code>·
                    <code>constants.js</code>로 분리하고 <B>Facade 패턴</B>으로
                    IPC 핸들러를 일괄 등록. 윈도우 크기·URL·패딩 등 매직 넘버를{" "}
                    <code>constants.js</code>로 중앙화하여 변경 포인트를 단일화
                  </>,
                ]}
              />
              ...
              <Project
                ...
                items={[
                  ...
                  <>
                    Stripe Webhook 비동기 처리 + 자체 DB 반영 지연으로 결제 직후
                    상세 내역을 즉시 표시할 수 없는 문제를 인식. 결제 완료
                    화면에서 서버 상태를 <B>폴링(5초 → 1초 최적화)</B>하며
                    Stripe↔서버 간 eventual consistency를 UX로 자연스럽게
                    흡수하고, debounce로 결제 버튼 중복 클릭에 의한 이중 과금을
                    원천 차단
                  </>,
                  <>
                    Google Sheets 기반 <B>번역 자동 빌드 파이프라인</B> + AWS
                    Secrets Manager 기반 시크릿 관리 도입으로 다국어·환경 설정
                    자동화
                  </>,
                ]}
              />
              ...
              <Project
                ...
                items={[
                  <>
                    키오스크(하드웨어)와 모바일 웹을 실시간 동기화하는 양방향
                    통신 설계. 스윙 영상 업로드 중 AI 응답이 동시 도착해 UI가
                    불안정해지는 문제를 <B>메시지 버퍼링 큐 아키텍처</B>로 해결
                  </>,
                  <>
                    외부 STT 라이브러리의 Web Worker 호환성 문제를 진단하고
                    네이티브 <B>MediaRecorder API</B>로 전환 — 269줄의 불필요한
                    의존성 제거, 상태머신 패턴의 <code>useSTT</code>로 중복 호출
                    버그 근본 해결
                  </>,
                  <>
                    Android/iOS 네이티브 브릿지 인터페이스(<code>AiBridge</code>
                    ) 설계, 백엔드·네이티브·키오스크 팀과 WebSocket 프로토콜 및
                    API 스펙 협의·통합
                  </>,
                ]}
              />
              ...
              <Project
                ...
                items={[
                  <>
                    모바일 화면 전체를 새로운 <B>디자인 시스템 10종</B>(Badge,
                    BottomSheet, Button, Card 등)으로 주도적 구축·적용하여 UI
                    전면 개편
                  </>,
                ]}
              />
          ...
          <div>
            ...
            <div className="pl-6 ml-[52px]">
              <Project
                ...
                items={[
                  <>
                    ...
                    <B>LCP 6.6s → 0.9s (86%↓)</B>, CLS 0.737 → 0 (100%↓)
                  </>,
                  <>
                    Zod 스키마 기반 유효성 검증 재설계, SEO
                    최적화(사이트맵·robots.txt·Schema.org) —{" "}
                    <B>Lighthouse SEO 83 → 92점</B>
                  </>,
                  <>
                    기기별 맞춤 번역 플러그인 자체 개발 — 윈도우 리사이즈
                    이벤트로 디바이스 타입 감지, 반응형 레이아웃 깨짐 문제 해결
                  </>,
                ]}
              />
              ...
              <Project
                ...
                items={[
                  <>
                    Highcharts 기반 유저 통계 시각화 차트 및 사용자별 맞춤형
                    필터 개발
                  </>,
                  <>
                    Intersection Observer API — Viewport 진입 차트만 호출하도록
                    최적화, <B>TBT 약 90% 단축</B>
                  </>,
                ]}
              />
              ...
              <Project
                ...
                items={[
                  <>
                    Preload 기법으로 load·open 함수 호출 분리 —{" "}
                    <B>광고 로딩 시간 60% 단축</B>, 연속 재생 UX 구현
                  </>,
                  <>
                    공통 Mixin 모듈 설계 — OS·Placement ID 상수화, 프로젝트 간
                    일관된 광고 호출 구조화
                  </>,
                ]}
              />
              ...
              <Project
                ...
                items={[
                  <>
                    사내 스터디에서 학습한 React를 실무 최초 적용 — Vue2 기반
                    결제 페이지를 <B>React + TypeScript로 마이그레이션</B>, API
                    응답 타입 안정성 확보로 런타임 에러 사전 예방
                  </>,
                ]}
              />
              ...
              <Project
                ...
                items={[
                  <>
                    <B>Optimistic Update</B> 적용 — 서버 응답 전 UI 즉시
                    반영으로 네트워크 지연 UX 개선
                  </>,
                  <>
                    Webpack 동적 import + webpackChunkName <B>코드 스플리팅</B>{" "}
                    — 초기 번들 크기 절감
                  </>,
                  <>
                    Android 웹뷰 키패드 하단 공백 이슈 해결 — User-Agent 감지로
                    Android 환경에서만 bottom 영역 0 처리
                  </>,
                ]}
              />
            </div>
